...
const Contact = function () {
	...
	function handleSubmit(e) {
		...
		try {
			window.open(
				`mailto:babskolawolejohnson@gmail.com ?subject=${contactInput.subject}&body=${contactInput.name}: ${contactInput.message}. My email is ${contactInput.email}`
			)
			setcontactInput({
				name: '',
				email: '',
	...
	return (
		<>
			<Helmet>
				<title>artkolawolejohnson: CONTACT-PAGE</title>
			</Helmet>
			<Layout>
					...
					<div className="bg-white flex flex-col md:flex-row justify-center items-center md:px-20 md:rounded-t-[50px]">
						<form
							...
							<p className="font-light text-left text-[14px] max-w-[80%] lg:max-w-[70%] my-5">
								At artkolawolejohnson, we are customer centric, we take pride in
								...
								{showResponse && (
									<div className="flex flex-col text-center">
										<span className="mt-5 text-yellow-600 font-light tracking-wide text-sm">
											Your enquiry has been submitted to artkolawolejohnson
										</span>
										<div className="flex flex-row w-[70%] mx-auto">
											...
											<span
												onClick={() => {
													navigate('/art-gallery/:artkolawolejohnson')
													localStorage.setItem('requestData', '')
												}}
												className="font-bold hover:cursor-pointer mx-auto text-[12px] mt-5 text-cyan-900 navStyleChild ">
												Back to Gallery
